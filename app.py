...
def venues():
    ...
    for venue in venues:
        num_upcoming_shows = db.session.query(Venue).join(Show).filter(
            Venue.id == venue.id, Show.start_time > datetime.now()
        ).count()
...
def show_venue(venue_id):
    ...
    upcoming_shows = Show.query.join(Venue).filter(Show.venue_id == venue_id).filter(
        Show.start_time>datetime.now()
    ).all()
    past_shows = Show.query.join(Venue).filter(Show.venue_id == venue_id).filter(
        Show.start_time < datetime.now()
    ).all()
    past_shows_data = []
    upcoming_shows_data = []
...
@app.route('/venues/create_new', methods=['POST'])
def create_venue_submission():
    ...
    try:
        ...
        for g in request.form.get('genres'):
            venue_data['genres'].append(g)
...
def search_artists():
    ...
    search_keyword = request.form.get('search_term', '')
    artists = Artist.query.filter(Artist.name.ilike('%'+search_keyword+'%')).all()
    response = {
        'count': len(artists),
        'data': artists
    }
...
def show_artist(artist_id):
    ...
    upcoming_shows = Show.query.join(Artist).filter(Show.artist_id == artist_id).filter(
        Show.start_time > datetime.now()
    ).all()
    past_shows = Show.query.join(Artist).filter(Show.artist_id == artist_id).filter(
        Show.start_time < datetime.now()
    ).all()
    past_shows_data = []
    upcoming_shows_data = []
...
def edit_artist_submission(artist_id):
    ...
    try:
        ...
        artist.looking_for_venue = True if request.form.get('looking_for_venue') is 'y' else False
...
def edit_venue_submission(venue_id):
    ...
    try:
        ...
        venue.seeking_talent = True if request.form.get('seeking_talent') is 'y' else False
...
def create_show_submission():
    ...
    error = False
    try:
        show_data = request.form.to_dict()
        show = Show(
